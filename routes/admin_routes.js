...
router.get("/review_product/:id",async function(req,res){
    ...
    product_category.product_category_id =product.product_category_id AND business.business_id = product.business_id AND product_id='${req.params.id}'`);
...
router.get("/approve_product/:id",async function(req,res){
    ...
    var sql=`UPDATE product SET product.status='approved', product.verified_date='${today}' WHERE product_id='${req.params.id}'`;
    var data=await exe(sql);
    // res.send(data);
